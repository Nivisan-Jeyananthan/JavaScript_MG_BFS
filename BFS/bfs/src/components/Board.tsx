...
export default function Board(props: IBoardProps) {
  const [squares, setSquares] = useState<SquareEnumType[]>(
    Array(props.boardWidth * props.boardLength).fill(null)
  );
  ...
  const checkGameObjects = () => {
    ...
    squares.forEach((square) => {
      if (square === SquareEnum.MAUS) setHasMouse(true);
      else if (square === SquareEnum.KÄSE) setHasCheese(true);
    });
  ...
  const renderLength = () => {
    ...
    return <div className="parentButton">{arrayLength}</div>;
  };
